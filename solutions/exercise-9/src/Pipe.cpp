...
void Pipe::push(std::unique_ptr<Event> in)
{
  ...
  if( (next_in+1)%array_size != next_out) {
    ...
    if(next_in == array_size) next_in = 0;
    while_empty.notify_all();
  }
}
...
std::unique_ptr<Event> Pipe::pull()
{
  ...
  //   while_empty.wait(guard);
  // }
  while_empty.wait(guard, [this]{ return next_out != next_in; }); 
  rv = std::move(eventList[next_out++]);
  if(next_out == array_size) next_out = 0;
  return rv;
}
